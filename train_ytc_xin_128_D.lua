...
opt = lapp[[
  ...
  --scale            (default 144)          scale of images to train on
...
if opt.network == '' then
  ...
  model_D:add(cudnn.SpatialMaxPooling(2,2))
  model_D:add(nn.SpatialDropout(0.2))
  model_D:add(nn.Reshape(9*9*96))
  model_D:add(nn.Linear(9*9*96, 1024))
  model_D:add(cudnn.ReLU(true))
...
--  local noise_inputs = torch.Tensor(N, opt.noiseDim)
  ...
  local noise_inputs = torch.Tensor(N, 3, 18, 18)
  --print(torch.squeeze(noise_input_high[{{idx}}]):size())
  for i = 1,N do
	--idx = math.random(nval) 
	idx = i  
	--noise_inputs[{{i}}] = noise_input_high[{{idx}}] 
    noise_inputs[{{i}}] = image.scale(torch.squeeze(noise_input_high[{{idx}}]),18,18) 
  end
  -- Generate samples
  -- print(torch.squeeze(noise_inputs[{{idx}}]):size()) 
...
-- training loop
while true do
  local to_plot = getSamples(valData_HR, 40)  
  torch.setdefaulttensortype('torch.FloatTensor')
  --print(valData_HR:size()) 
  ...
  --print(trainData_LR:size())
  --print(trainData_HR:size())
  -- train/test
