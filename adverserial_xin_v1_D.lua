...
function adversarial.train(dataset_LR,dataset_HR)

  model_G:training()
  model_D:training()
  epoch = epoch or 1
  local N = dataset_HR:size()[1] 
...
--	local LR_inputs = dataset_LR[{{t,math.min(t+dataBatchSize,dataset_LR:size()[1])-1}}]
	...
	local LR_inputs = torch.Tensor(opt.batchSize, 3, 18, 18)
    ...
    for k=1,opt.K do
	  ...
	  --print(dataset_HR:size())  
		...
		local sample = torch.Tensor(dataBatchSize, 3, 18, 18)
		local k = 1

		for i = t+dataBatchSize,t+opt.batchSize-1 do
		  --print('i',i)
		  --print(dataset_LR:size())  	
		  local sample_LR = dataset_LR[IDX[i]]

		  --print(sample:clone():size()) 	  
...
function adversarial.test(dataset_LR,dataset_HR, N)
  ...
  for t = 1,N,opt.batchSize do
    -- display progress
    xlua.progress(t, dataset_LR:size()[1])
    ...
    local inputs_lr = torch.Tensor(opt.batchSize, 3, 18, 18)
    local k = 1
    for i = t,t+opt.batchSize-1 do
      local idx = math.random(dataset_HR:size()[1])
